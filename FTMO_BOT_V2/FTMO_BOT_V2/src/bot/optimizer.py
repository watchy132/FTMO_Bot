...
def save_results_csv(
    results: List[Tuple[int, int, dict]], filename: str = "grid_results.csv"
):
...
def plot_heatmap(results: List[Tuple[int, int, dict]], filename: str = "heatmap.png"):
    ...
    for short, long, res in results:
        grid[short_index[short], long_index[long]] = res.get(
...
def get_top_results(
    ...
    flat_sorted = sorted(
        flat, key=lambda x: (x[3] if x[3] is not None else float("-inf")), reverse=True
    )
...
def save_top_csv(
    top_results: List[Tuple[int, int, dict]], filename: str = "top_results.csv"
):
    with open(filename, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["rank", "short", "long", "sharpe", "cagr", "total_return", "max_drawdown"]
        )
...
def walk_forward_evaluate(
    ...
    metrics_acc = {
        combo: {"sharpe": [], "cagr": [], "total_return": [], "max_drawdown": []}
        for combo in combos
    }
...
def save_walkforward_csv(
    aggregated_results: List[Tuple[int, int, dict]],
    filename: str = "walkforward_results.csv",
):
