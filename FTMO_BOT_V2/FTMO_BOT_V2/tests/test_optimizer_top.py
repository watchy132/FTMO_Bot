...
def test_get_top_results():
    ...
    results = grid_search_ma(prices, short_windows=[3,5,7], long_windows=[20,30], initial_capital=10000)
    top = get_top_results(results, top_n=3, sort_key='sharpe')
    assert len(top) == 3
    # check ordering descending by the chosen metric
    metrics = [res.get('sharpe') if res.get('sharpe') is not None else res.get('total_return') for _, _, res in top]
