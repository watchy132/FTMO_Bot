...
async def normalize(request: Request):
    ...
    return {"ok": True, "setups": valids, "reasons": reasons, "ts": __import__("time").time_ns() // 1_000_000}
...
async def _decide(request: Request):
    ...
    try:
        ...
        return {
            "ok": True,
            "decisions": [{"action": "skip", "reason": f"engine_error:{e}", "setups": []}],
            "ts": __import__("time").time_ns() // 1_000_000,
        }
    return {
        "ok": True,
        "decisions": [{"action": "preview", "reason": (reasons[0] if reasons else "OK"), "setups": valids}],
        "ts": __import__("time").time_ns() // 1_000_000,
    }
