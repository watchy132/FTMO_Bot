...
def _llm_decide(symbol: str) -> Tuple[Optional[Dict[str, Any]], str]:
    ...
    user_msg = (
        f"Décide sur {symbol} en M5 maintenant. Donne symbol, direction, entry, sl, tp, reason au format JSON strict."
    )
...
def decide(payload: Dict[str, Any], timeout: Optional[int] = None, **kwargs) -> Dict[str, Any]:
    ...
    return _ok(
        {"decisions": [{"action": setup["direction"].lower(), "reason": setup.get("reason", "llm"), "setups": [setup]}]}
    )
...
def normalize_setup_v2(s: Dict[str, Any]) -> Tuple[Dict[str, Any], str]:
    ...
    tp = _to_float(
        s.get("tp") or s.get("take_profit") or s.get("target") or s.get("takeprofit") or s.get("targetPrice")
    )
...
def normalize_setups_v3(setups):
    ...
    if isinstance(setups, str):
        obj = (
            (_json_try_block if "_json_try_block" in globals() else _json_try_local)(setups)
    ...
    if isinstance(setups, dict):
        ...
        if isinstance(ext, str):
            obj = (
                (_json_try_block if "_json_try_block" in globals() else _json_try_local)(ext)
                if "_json_try_block" in globals()
    ...
    if chs:
        ...
        for t in tcs:
            ...
            if isinstance(args, str):
                v = (
                    (_json_try_block if "_json_try_block" in globals() else _json_try_local)(args)
                    if "_json_try_block" in globals()
