...
if not hasattr(__bi, "_JSON_RE"):
    __bi._JSON_RE = __rb.compile(r"(\{[\s\S]*\}|\[[\s\S]*\])", __rb.S)
try:
    _JSON_RE
except NameError:
    _JSON_RE = __bi._JSON_RE
...
def _llm_decide(symbol: str) -> Tuple[Optional[Dict[str, Any]], str]:
    ...
    if d in ("LONG", "ACHAT", "BUY"):
        raw["direction"] = "BUY"
    elif d in ("SHORT", "VENTE", "SELL"):
        raw["direction"] = "SELL"
    # entry fallback
    ...
    if err:
        return None, f"normalize_error:{err}"
...
def decide(
    ...
    if dbg in ("buy", "sell"):
        mock = {
            ...
            "direction": dbg.upper(),
            "entry": 1.100,
            "sl": 1.098 if dbg == "buy" else 1.102,
            "tp": 1.104 if dbg == "buy" else 1.096,
            "source": "debug_force",
        }
        setup, err = normalize_setup(mock)
        if err:
            return _ok({"decisions": [], "status": "SKIP", "why": err})
        return _ok(
            {
                "decisions": [
                    {"action": dbg, "reason": f"debug_force {dbg}", "setups": [setup]}
                ]
            }
        )
    ...
    return _ok(
        {
            "decisions": [
                {
                    "action": setup["direction"].lower(),
                    "reason": setup.get("reason", "llm"),
                    "setups": [setup],
                }
            ]
        }
...
def normalize_setup_disabled(*a, **k):
    return (
        ({}, "REJECT: disabled")
        if "normalize_setup" == "normalize_setup"
        else ([], ["REJECT: disabled"])
    )
...
def normalize_setups_disabled(*a, **k):
    return (
        ({}, "REJECT: disabled")
        if "normalize_setups" == "normalize_setup"
        else ([], ["REJECT: disabled"])
    )
...
def _norm_direction(x: Any) -> Union[str, None]:
    if not x:
        return None
    s = str(x).strip().lower()
    return _DIR_ALIASES.get(s) or (
        s.upper() if s in ("buy", "sell", "BUY", "SELL") else None
    )
...
def _compute_tp(entry: float, sl: float, direction: str, rrr: float) -> float:
    return (
        entry + rrr * abs(entry - sl)
        if direction == "BUY"
        else entry - rrr * abs(entry - sl)
    )
...
# ---- Freeform "PLACE BUY ..." ----
_FREEFORM_DIR_SYM_RE = re.compile(
    r"(?i)\b(?:PLACE\s+)?(BUY|SELL)\s+([A-Z0-9._/]{3,20})"
)


def _grab_num(label, text):
    m = re.search(
        r"(?i)\b" + re.escape(label) + r"\s*[:=]\s*([-+]?\d+(?:\.\d+)?)", text
    )
...
def _parse_freeform_setups(text):
    ...
    sl = (
        _grab_num("sl", text) or _grab_num("stop", text) or _grab_num("stop_loss", text)
    )
    tp = (
        _grab_num("tp", text)
        or _grab_num("take_profit", text)
        or _grab_num("target", text)
    )
    rrr = (
        _grab_num("rrr", text)
        or _grab_num("rr", text)
        or _grab_num("reward_risk", text)
    )
    lots = (
        _grab_num("lots", text)
        or _grab_num("size", text)
        or _grab_num("quantity", text)
    )
...
def normalize_setup_v2(s: Dict[str, Any]) -> Tuple[Dict[str, Any], str]:
    ...
    tp = _to_float(
        ...
        or s.get("target")
        or s.get("takeprofit")
        or s.get("targetPrice")
    )
    rrr = _to_float(
        s.get("rrr") or s.get("rr") or s.get("risk_reward") or s.get("reward_risk")
    )
    ...
    if not _valid_side(entry, sl, tp, direction):
        return {}, f"REJECT: inconsistent levels for {direction}"
    ...
    if isinstance(setups, dict):
        for k in ("setups", "candidates"):
            if isinstance(setups.get(k), list):
                setups = setups[k]
                break
        if isinstance(setups, dict) and isinstance(
            setups.get("decision", {}).get("setups"), list
        ):
            setups = setups["decision"]["setups"]

    if isinstance(setups, list):
        for i, item in enumerate(setups):
            norm, why = normalize_setup_v2(
                item if isinstance(item, dict) else {"symbol": None}
            )
            if norm:
                valids.append(norm)
...
def _grab_num(label, text):
    ...
    m = _re.search(
        r"(?i)\b" + _re.escape(label) + r"\s*[:=]\s*([-+]?\d+(?:\.\d+)?)", text
    )
    return float(m.group(1)) if m else None
...
def normalize_setup_v2(s: Dict[str, Any]) -> Tuple[Dict[str, Any], str]:
    ...
    if (tp is None or tp <= 0) and (rrr and rrr > 0):
        tp = _compute_tp(entry, sl, direction, rrr)
    if (rrr is None or rrr <= 0) and (tp and tp > 0):
        rrr = _calc_rrr(entry, sl, tp)
...
def normalize_setups_v3(setups):
    ...
    if isinstance(setups, str):
        obj = (
            (_json_try_block if "_json_try_block" in globals() else _json_try_local)(
                setups
            )
            if "_json_try_block" in globals()
            else _json_try_local(setups)
        )
        ...
        if isinstance(ext, str):
            obj = (
                (
                    _json_try_block
                    if "_json_try_block" in globals()
                    else _json_try_local
                )(ext)
                if "_json_try_block" in globals()
                else _json_try_local(ext)
            )
...
def _unwrap_order_like(d):
    ...
    if (
        ...
        or any(
            k in cand
            for k in ("sl", "tp", "stop", "take_profit", "entry", "price", "target")
        )
    ):
        out = dict(cand)
...
def _order_like_to_setup(d):
    ...
    if (
        ...
        or any(
            k in cand
            for k in ("entry", "price", "sl", "stop", "tp", "take_profit", "target")
        )
    ):
        out = dict(cand)
        if side and isinstance(side, str):
