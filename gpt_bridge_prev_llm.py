...
def normalize_setup(s: Dict[str, Any]) -> Tuple[Dict[str, Any], str]:
    ...
    sl = _to_float(
        s.get("sl") or s.get("stop_loss") or s.get("stop") or s.get("sl_price")
    )
    tp = _to_float(
        s.get("tp") or s.get("take_profit") or s.get("target") or s.get("tp_price")
    )
...
def decide(
    payload: Dict[str, Any], timeout: float | None = None, **kwargs
) -> Dict[str, Any]:
    ...
    if dbg in ("buy", "sell"):
        ...
        return _ok(
            {"status": "TAKE", "why": f"debug_force {dbg}", "decisions": [setup]}
        )
