...
def get_symbol_spec(symbol: str) -> Dict[str, Any]:
    ...
    return dict(
        digits=info.digits, point=info.point, trade_contract_size=info.trade_contract_size,
...
def make_order_request(symbol: str, volume: float, side: str, price: float,
                       sl: Optional[float], tp: Optional[float], deviation: int = 10) -> Dict[str, Any]:
    ...
    return {
        ...
        "type": ord_type, "price": float(price), "sl": float(sl or 0.0), "tp": float(tp or 0.0),
        "deviation": deviation, "type_filling": mt5.ORDER_FILLING_FOK, "type_time": mt5.ORDER_TIME_GTC,
