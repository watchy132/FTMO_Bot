...
EXPOSURE_CAP_PCT = float(
    os.getenv("EXPOSURE_CAP_PCT", "0.015")
)  # plafond d’exposition par trade
...
def exposure_check(risk_usd: float) -> tuple[bool, str]:
    ...
    if risk_usd > cap_usd + 1e-9:
        return (
            False,
            f"exposure_exceeds_cap risk_usd={risk_usd:.2f} cap_usd={cap_usd:.2f}",
        )
    return True, "ok"
...
def evaluate(setup_dict: Dict[str, Any], instr_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if setup.rrr is not None and setup.rrr < MIN_RRR_TRADE:
        journal(
            "REJECT",
            f"rrr_below_min:{setup.rrr:.2f}<{MIN_RRR_TRADE}",
            setup,
            0.0,
            0.0,
            0.0,
    ...
    lots, risk_usd, reward_usd = compute_lot_from_risk(
        setup, instr, ACCOUNT_EQUITY_USD, RISK_PER_TRADE_PCT
    )

    if lots <= 0.0 or risk_usd <= 0.0:
        journal(
            "REJECT", "non_positive_lot_or_risk", setup, lots, risk_usd, reward_usd, {}
        )
