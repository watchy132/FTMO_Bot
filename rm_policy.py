...
def lot_size(equity, risk_pct, sl_pips, pip_value, lot_step=0.01, min_lot=0.01, max_lot=100.0):
...
def rr_effectif(sl_pips, tp_rr, spread_pips, commission_per_lot, slippage_pips, pip_value, lot):
...
def validate_setup(equity, risk_pct, sl_pips, tp_rr,
                   spread_pips, commission_per_lot, slippage_pips,
                   pip_value, lot_step=0.01, min_lot=0.01, max_lot=100.0,
                   rr_min=2.0, rr_buffer=0.05):
    lot = lot_size(equity, risk_pct, sl_pips, pip_value, lot_step, min_lot, max_lot)
    rr_eff = rr_effectif(sl_pips, tp_rr, spread_pips, commission_per_lot, slippage_pips, pip_value, lot)
