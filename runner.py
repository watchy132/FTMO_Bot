#!/usr/bin/env python3
# ##__JSON_RE_FALLBACK__##
try:
    _JSON_RE
except NameError:
    _JSON_RE = __r_fb.compile(r"(\{[\s\S]*\}|\[[\s\S]*\])", __r_fb.S)
# -*- coding: utf-8 -*-

from __future__ import re as __r_fb, annotations
...
TRADER_SCRIPT = str(
    (pathlib.Path(__file__).parent / "scripts" / "ftmo_trader_wrap.py").resolve()
)
...
def _is_trader_cmd_tokens(tokens):
    ...
    return any(
        ("FTMO_GPT_Trader_MAIN.py" in str(t)) or (ts and str(t) == str(ts))
        for t in tokens
    )
...
def _ensure_entry_list(tokens, loc):
    ...
    if "--entry" in tokens:
        return tokens
    if not _is_trader_cmd_tokens(tokens):
        return tokens
...
def _ensure_entry_str(cmd, loc):
    if not isinstance(cmd, str):
        return cmd
    ts = globals().get("TRADER_SCRIPT")
    if ("FTMO_GPT_Trader_MAIN.py" not in cmd) and (not ts or str(ts) not in cmd):
        return cmd
...
def _patch_cmd_args(a, kw):
    ...
    if "args" in kw:
        if isinstance(kw["args"], list):
            ...
            try:
                print("[EXEC]", " ".join(str(x) for x in kw["args"]))
            except Exception:
                pass
...
_sp.run = _wrap_run
subprocess.run = _wrap_run
_sp.call = _wrap_call
subprocess.call = _wrap_call
_sp.Popen = _wrap_popen
