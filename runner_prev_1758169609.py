...
def _caller_locals():
    ...
    for _ in range(8):
        if not f: break
        loc = f.f_locals
        if isinstance(loc, dict) and ('setups' in loc or 's' in loc):
            return loc
        f = f.f_back
    return {}
...
def _find_entry(loc, symbol=None):
    try:
        s = loc.get('s')
        if isinstance(s, dict) and s.get('entry') is not None:
            if not symbol or s.get('symbol') in (None, symbol) or s.get('symbol') == symbol:
                return s['entry']
    except Exception: pass
    try:
        for st in (loc.get('setups') or []):
            if isinstance(st, dict) and st.get('entry') is not None:
                if not symbol or st.get('symbol') in (None, symbol) or st.get('symbol') == symbol:
                    return st['entry']
    except Exception: pass
    return None
...
def _maybe_add_entry(args, loc):
    try:
        ...
        if '--entry' in args: return args
        # TRADER_SCRIPT présent ?
        try:
            if 'TRADER_SCRIPT' in globals() and globals()['TRADER_SCRIPT'] not in args:
                return args
        except Exception:
            pass
        sym = _extract_symbol(args)
        e = _find_entry(loc, sym)
        if e is None: return args
        # insère avant --sl si possible, sinon en fin
        try:
            idx = args.index('--sl')
        except ValueError:
            idx = len(args)
