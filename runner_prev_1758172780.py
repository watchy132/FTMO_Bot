...
def _find_entry(loc, symbol=None):
    def _iter(obj):
        if isinstance(obj, dict):
            ...
            for v in obj:
                yield from _iter(v)
    sym_up = (None if symbol is None else str(symbol).upper())
    for d in _iter(loc):
        try:
            if 'entry' in d and d.get('entry') is not None:
                sym = d.get('symbol')
                if sym_up is None or sym is None or str(sym).upper() == sym_up:
                    return d['entry']
        except Exception:
            pass
    return None
    ...
    if a and isinstance(a[0], list):
        ...
        try:
            ...
            try:
                ...
                if isinstance(_args, list) and _entry is not None and '--entry' not in _args:
                    try:
                        _idx = _args.index('--sl')
                    except ValueError:
                        _idx = len(_args)
...
def main(argv: List[str]|None=None) -> int:
    ...
    try:
        _lv = locals()
        _entry = None
        if 's' in _lv and isinstance(_lv['s'], dict) and _lv['s'].get('entry') is not None:
            _entry = _lv['s']['entry']
        if _entry is None:
            for _st in (_lv.get('setups') or []):
                if isinstance(_st, dict) and _st.get('entry') is not None:
