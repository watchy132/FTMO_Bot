...
try:
    ...
    print("MetaTrader5 non disponible:", e); raise SystemExit(1)
...
WHITELIST = json.loads(os.getenv("WHITELIST", '["EURUSD","GBPUSD","USDJPY","XAUUSD","US100.cash","US30.cash","BTCUSD"]'))
...
if USE_GPT:
    try:
        ...
        print("GPT désactivé:", e); USE_GPT = False

# ---------------- Utils ----------------
def utc_now_iso(): return datetime.utcnow().replace(tzinfo=timezone.utc).isoformat()
...
def _parse_hhmm(s):
    h,m = map(int, s.split(":")); now = datetime.utcnow()
    return now.replace(hour=h, minute=m, second=0, microsecond=0)

def in_session_utc():
    now = datetime.utcnow().time()
    for start, end in SESSIONS_UTC.values():
        if _parse_hhmm(start).time() <= now <= _parse_hhmm(end).time(): return True
    return False

class ExecGuard:
    def __init__(self, limit=3): self.err_streak=0; self.limit=limit
    def ok(self): self.err_streak=0
    def ko(self): self.err_streak+=1
    def blocked(self): return self.err_streak>=self.limit

# ---------------- MT5 ----------------
def init_mt5():
    if mt5.initialize(): return True
    try: os.startfile(MT5_TERMINAL_PATH)
    except Exception as e: print("Lancement MT5 direct impossible:", e)
    time.sleep(3); return mt5.initialize()
...
def ensure_symbol(symbol):
    ...
    if not si or not si.visible:
        if not mt5.symbol_select(symbol, True): raise RuntimeError(f"Impossible d’activer {symbol}")
        si = mt5.symbol_info(symbol)
    return si

def pip_value_per_lot(si):
    tick_val = getattr(si,"trade_tick_value",0.0) or getattr(si,"tick_value",0.0)
    tick_size= getattr(si,"trade_tick_size",0.0) or getattr(si,"tick_size",0.0) or si.point
    if tick_size<=0: tick_size=si.point
...
def symbol_spread_pips(symbol):
    ...
    if not si or not tick: return None
...
def log_trade(row): append_csv(LOG_TRADES_CSV, row)
def log_equity(equity, dd_total):
    append_csv(LOG_EQUITY_CSV, {"ts": utc_now_iso(),"equity": equity,"dd_total": dd_total})
...
def calc_lot_by_risk(*, equity, risk_pct, entry, sl, si,
                     spread_pips=0.0, avg_slippage_pips=0.0,
                     commission_per_lot=0.0, hold_days=0.0, daily_swap_per_lot=0.0):
    risk_cash = equity * (risk_pct/100.0)
    dist_points = abs(entry - sl) / si.point
    if dist_points<=0: return si.volume_min
    ...
    risk_per_lot = max(1e-9, (stop_pips*pip_val)+spread_cost+slip_cost+commission_per_lot+swap_cost)
...
def place_market(symbol, side, lots, sl, tp):
    si = mt5.symbol_info(symbol); tick = mt5.symbol_info_tick(symbol)
    if not si or not tick: raise RuntimeError("symbol/tick indisponible")
    ...
    req = {
        ...
        "type": order_type, "price": float(price_req), "sl": float(sl), "tp": float(tp),
        "deviation": 10, "type_filling": mt5.ORDER_FILLING_IOC, "comment": RUN_TAG
    }
    res = mt5.order_send(req)
    if res is None: raise RuntimeError("order_send None")

    fill = float(getattr(res, "price", 0.0) or 0.0)
    if fill<=0.0: fill = float(price_req)
...
def run_trading_cycle(guard: ExecGuard):
    ai = mt5.account_info()
    if not ai: raise RuntimeError("Pas d’account_info")
    equity = float(ai.equity)
    dd_total = max(0.0, (ACCOUNT_EQUITY_BASE - equity)/ACCOUNT_EQUITY_BASE)
    print(f"Equity={ai.equity:.2f} Balance={ai.balance:.2f} FreeMargin={ai.margin_free:.2f}")
    # ... reste identique ...
