...
def connect():
    ...
    if not info:
        raise SystemExit(f"MT5 not authorized: {mt5.last_error()}")
    return info
...
def place(
    symbol, side, lots, sl=None, tp=None, entry=None, pending=None, comment="NO_EA"
):
    ensure_symbol(symbol)
    lots = float(lots)
    lots = lot_round(symbol, lots)
    etype = order_type(side, pending)
    price = None
    if pending:
        if entry is None:
            raise SystemExit("pending order requires --entry")
        price = float(entry)
    else:
    ...
    req = {
        "action": (
            mt5.TRADE_ACTION_DEAL if pending is None else mt5.TRADE_ACTION_PENDING
        ),
        "symbol": symbol,
        "type": etype,
        "volume": lots,
    ...
    if r is None:
        raise SystemExit(f"order_send None: {mt5.last_error()}")
...
def main():
    ...
    res = place(
        a.symbol, a.side, a.lots, a.sl, a.tp, a.entry, a.pending, comment="NO_EA"
    )
