...
def guardrails_allow(a, ai, prof, entry, lots):
    ...
    if used_today_loss + pot_loss >= max_daily_loss_amt:
        log(
            f"BLOCKED: daily loss {used_today_loss:.2f} + potential {pot_loss:.2f} > limit {max_daily_loss_amt:.2f}"
        )
        return False
    min_equity = ref_balance - max_total_loss_amt
    if (ai.equity - pot_loss) < min_equity:
        log(
            f"BLOCKED: equity after potential loss {ai.equity - pot_loss:.2f} < min equity {min_equity:.2f}"
        )
        return False
    return True
...
def send_order(a):
    ...
    risk_pct = (
        ...
        else float(prof["risk_per_trade_pct"])
    )
    risk_amount = float(ai.balance) * (risk_pct / 100.0)
    lots = (
        float(a.lots)
        if a.lots
        else lots_from_risk(a.symbol, entry, float(a.sl), risk_amount)
    )
