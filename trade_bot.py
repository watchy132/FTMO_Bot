...
def init_mt5():
    ...
    if account_info is None:
        raise RuntimeError(
            "MT5 account not logged in (ouvrir la session dans le terminal)."
        )
    return account_info
...
def compute_lot_for_risk(
    symbol: str, entry: float, sl: float, max_risk_pct: float
) -> float:
    eq = equity_usd()
...
def order_type_from_setup(
    side: str, entry_type: str, entry: float, snapshot: Dict[str, float]
):
    side = side.lower()
...
def send_order(setup: Dict) -> Dict:
    ...
    if lots <= 0:
        return {
            "status": "blocked",
            "reason": "lot=0 (distance SL trop faible ou equity nulle)",
        }
    ...
    if trade_risk_pct > MAX_RISK_PCT + 1e-6:
        return {
            "status": "blocked",
            "reason": f"expo dépasserait {MAX_RISK_PCT}% (calc={trade_risk_pct:.2f}%)",
        }
    ...
    if open_positions_count() >= MAX_OPEN_TRADES:
        return {
            "status": "blocked",
            "reason": f"max {MAX_OPEN_TRADES} trades déjà ouverts",
        }
    ...
    request = {
        ...
        "price": (
            ...
            else (snap["ask"] if side.lower() == "buy" else snap["bid"])
        ),
        "sl": sl,
        "tp": tp,
        "deviation": 50,
